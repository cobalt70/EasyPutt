...
`ContentView2.swift:167-226`의 버튼 구성을 **+, Reset, 설정 3개**로 줄인다:
...
- **+** (캡처) — 유지
- **Reset** — 유지. `showNormalsList` 리셋 코드(`ContentView2.swift:220`)는 state 자체가 삭제되므로 같이 제거. `showResults` 리셋 코드는 시트 쪽 state로 유지.

### 2.4 설정 시트 (신규)

두 섹션으로 구성한다:

1. **스팀프미터** — 지금 상단 박스에 있던 스테퍼(`ContentView2.swift:76-88`, `－/＋` 버튼 + "Stimpmeter: X.XXm" 텍스트)를 그대로 옮긴다. 인터랙션/로직 변경 없음 — 위치만 이동.
2. **고급(결과 비교)** — 접혀있는 섹션(예: `DisclosureGroup`)으로, 펼치면 지금 "결과" 패널 내용(백+포워드/백워드전용 각각의 speed, 좌우 조준 cm, 경계 A/B, 소요시간)이 그대로 나온다. `ContentView2.swift:116-147`의 기존 뷰 코드를 거의 그대로 옮겨온다 — 로직 변경 없이 위치만 이동.
